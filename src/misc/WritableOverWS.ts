...
export class WritableOverWS extends Writable {
    ...
    if (!this._ws) {
      this.once("_ws", () =>
        this._ws!.send(
          Buffer.concat([
            this._fullCBBuffer!,
            Buffer.from([STREAM_FINISH_FLAG]),
          ]),
          {
            binary: true,
          },
  ...
  _write(
    ...
    if (this._ws.readyState !== 1) {
      callback &&
        callback(
          new Error(
            "Underlying socket has benn closed. Stream can not write data anymore"
          )
        );
      this.destroy(
        new Error(
  ...
  _bindWS(ws: WebSocket) {
    ...
    this.once(
      "close",
      () => this._ws && this._ws.removeListener("close", onWSClose)
    );
  }
  _setBPID(bpid: number) {
    ...
    this._receiver.on(this._bpEvent!, (type: number, chunk: Buffer) => {
      if (type === STREAM_BP_QUOTA_FLAG) {
        ...
        this.emit(
          "error",
          new Error(`Remote readable stream sent error: ${chunk.toString()}`)
        );
      }
    });
    this.once("close", () => {
  ...
  _setSID(sid: number) {
    ...
    this._fullCBBuffer = Buffer.concat([
      Buffer.from([TYPE_STREAM_MSG]),
      lengthBuffer,
      cbBufferSid,
    ]);
  }
