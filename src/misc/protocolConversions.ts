...
/**@internal */
export function serializeStreamRequest(
  type: number,
  role: string = "",
  event: string = "",
  cb: number,
...
/**@internal */
export function serializeStreamResponse(
  type: number,
  cb: number,
  ctr: number,
  data: any
...
export function serializeString(type: number, data: string) {
  return Buffer.concat([Buffer.from([type]), Buffer.from(data)]);
}
...
export function parseStreamResponse(buf: Buffer): InitialStreamContext {
  ...
  (<InitialStreamContext>ctx)._ctr = bufferToNumber(
    buf.slice(ctrFrom, payloadFrom)
  );
...
function markDataType(data: any): Buffer {
  ...
  switch (typeof data) {
    case "number":
      return Buffer.concat([
        Buffer.from([DATATYPE_NUMBER]),
        Buffer.from((data as number).toString()),
      ]);
    case "string":
