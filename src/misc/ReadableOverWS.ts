...
export class ReadableOverWS extends Readable {
  ...
  _sendQuota(quota: number): void {
    this._ws &&
      ...
      this._ws.send(
        Buffer.concat([
          this._fullQuotaBuffer!,
          Buffer.from([STREAM_BP_QUOTA_FLAG]),
          numberToBuffer(quota),
        ])
      );
  ...
  _setSID(sid: number) {
    ...
    let onPayload = (type: number, chunk: Buffer): void => {
      if (type === STREAM_CHUNK_FLAG) {
        ...
        let error = new Error(
          `Remote writable stream emitted error: ${chunk.toString()}`
        );
  ...
  _setBPID(bpid: number) {
    ...
    this._fullQuotaBuffer = Buffer.concat([
