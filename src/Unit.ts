...
export class Unit extends EventEmitter {
    ...
    /**@internal */
    request(params: MessageHeaders, data: any, cb?: (err: Error | null, ctx: Context) => void): Promise<Context> {
        if (cb) {
            sendRequest.call(this, params, data, cb);
        } else {
            return new Promise((resolve, reject) =>
                sendRequest.call(this, params, data, (err: Error | null, res: Context) => (err ? reject(err) : resolve(res)))
            );
        }
...
function sendRequest(this: Unit, params: MessageHeaders, data: any, cb: (err: Error | null, ctx: Context) => void, type = TYPE_REQ) {
    let cbid = this._cbid++;
...
function sendRequestForStream(this: Unit, params: MessageHeaders, data: any, type: number, ctr: number, cb: (err: Error | null, ctx: InitialStreamContext) => void) {
    let cbid = this._cbid++;
...
function handleMessage(this: Unit, ctx: Context) {
    ...
    this._peer._roles.has(ctx.role!) && this._peer._roles.get(ctx.role!)!._emitMsg(<Context>ctx);
}
